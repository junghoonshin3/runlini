// 기록 레이스 라이브 이벤트 안정화와 중복 억제를 담당하는 엔진
import 'package:flutter/foundation.dart';
import 'package:runlini/features/record_race/types/record_race_frame.dart';

enum RecordRaceEventType {
  offRoute,
  backOnRoute,
  overtake,
  lostLead,
  last500m,
  last200m,
  completed,
}

@immutable
class RecordRaceEvent {
  const RecordRaceEvent({required this.type, required this.frame});

  final RecordRaceEventType type;
  final RecordRaceFrame frame;
}

class RecordRaceEventEngine {
  RecordRaceEventEngine({
    this.offRouteStableDuration = const Duration(seconds: 10),
    this.leadStableDuration = const Duration(seconds: 15),
  });

  final Duration offRouteStableDuration;
  final Duration leadStableDuration;

  String? _sessionId;
  DateTime? _offRouteSince;
  DateTime? _backOnRouteSince;
  bool _offRouteAlerted = false;
  bool _backOnRouteAlerted = false;
  RecordRaceStatus? _leadCandidate;
  DateTime? _leadCandidateSince;
  RecordRaceStatus? _stableLeadStatus;
  bool _last500mAlerted = false;
  bool _last200mAlerted = false;
  bool _completedAlerted = false;

  List<RecordRaceEvent> eventsFor({
    required String sessionId,
    required RecordRaceFrame? frame,
    required bool isRunning,
    required DateTime now,
    bool completionPending = false,
  }) {
    if (_sessionId != sessionId) {
      reset();
      _sessionId = sessionId;
    }
    if (!isRunning || frame == null) {
      return const <RecordRaceEvent>[];
    }
    if (!frame.startConfirmed || frame.status == RecordRaceStatus.unavailable) {
      return const <RecordRaceEvent>[];
    }

    final events = <RecordRaceEvent>[];
    events.addAll(_routeEvents(frame, now));
    events.addAll(_leadEvents(frame, now));
    events.addAll(_finalStretchEvents(frame));
    if (completionPending && !_completedAlerted) {
      _completedAlerted = true;
      events.add(
        RecordRaceEvent(type: RecordRaceEventType.completed, frame: frame),
      );
    }
    return events;
  }

  void reset() {
    _sessionId = null;
    _offRouteSince = null;
    _backOnRouteSince = null;
    _offRouteAlerted = false;
    _backOnRouteAlerted = false;
    _leadCandidate = null;
    _leadCandidateSince = null;
    _stableLeadStatus = null;
    _last500mAlerted = false;
    _last200mAlerted = false;
    _completedAlerted = false;
  }

  List<RecordRaceEvent> _routeEvents(RecordRaceFrame frame, DateTime now) {
    final events = <RecordRaceEvent>[];
    if (frame.isOffRoute || frame.status == RecordRaceStatus.offRoute) {
      _backOnRouteSince = null;
      _backOnRouteAlerted = false;
      _offRouteSince ??= now;
      if (!_offRouteAlerted &&
          now.difference(_offRouteSince!) >= offRouteStableDuration) {
        _offRouteAlerted = true;
        events.add(
          RecordRaceEvent(type: RecordRaceEventType.offRoute, frame: frame),
        );
      }
      return events;
    }

    _offRouteSince = null;
    if (_offRouteAlerted && !_backOnRouteAlerted) {
      _backOnRouteSince ??= now;
      if (now.difference(_backOnRouteSince!) >= offRouteStableDuration) {
        _backOnRouteAlerted = true;
        _offRouteAlerted = false;
        events.add(
          RecordRaceEvent(type: RecordRaceEventType.backOnRoute, frame: frame),
        );
      }
    }
    return events;
  }

  List<RecordRaceEvent> _leadEvents(RecordRaceFrame frame, DateTime now) {
    final status = frame.status;
    if (status != RecordRaceStatus.ahead && status != RecordRaceStatus.behind) {
      _leadCandidate = null;
      _leadCandidateSince = null;
      return const <RecordRaceEvent>[];
    }

    if (_leadCandidate != status) {
      _leadCandidate = status;
      _leadCandidateSince = now;
      return const <RecordRaceEvent>[];
    }

    final since = _leadCandidateSince ?? now;
    if (now.difference(since) < leadStableDuration ||
        _stableLeadStatus == status) {
      return const <RecordRaceEvent>[];
    }

    final previous = _stableLeadStatus;
    _stableLeadStatus = status;
    if (previous == null) {
      return const <RecordRaceEvent>[];
    }

    return [
      RecordRaceEvent(
        type: status == RecordRaceStatus.ahead
            ? RecordRaceEventType.overtake
            : RecordRaceEventType.lostLead,
        frame: frame,
      ),
    ];
  }

  List<RecordRaceEvent> _finalStretchEvents(RecordRaceFrame frame) {
    if (frame.isOffRoute ||
        frame.status == RecordRaceStatus.offRoute ||
        !frame.distanceToFinishM.isFinite ||
        frame.totalRouteDistanceM <= 0) {
      return const <RecordRaceEvent>[];
    }

    final events = <RecordRaceEvent>[];
    if (!_last500mAlerted &&
        frame.totalRouteDistanceM > 500 &&
        frame.distanceToFinishM <= 500) {
      _last500mAlerted = true;
      events.add(
        RecordRaceEvent(type: RecordRaceEventType.last500m, frame: frame),
      );
    }
    if (!_last200mAlerted &&
        frame.totalRouteDistanceM > 200 &&
        frame.distanceToFinishM <= 200) {
      _last200mAlerted = true;
      events.add(
        RecordRaceEvent(type: RecordRaceEventType.last200m, frame: frame),
      );
    }
    return events;
  }
}
