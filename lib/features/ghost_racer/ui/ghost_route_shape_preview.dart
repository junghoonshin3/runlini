// 고스트 기록 선택에서 경로 모양 미리보기를 그리는 위젯
...
class GhostRouteShapePreview extends StatelessWidget {
  const GhostRouteShapePreview({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Container(
      key: const Key('ghost-route-shape-preview'),
      height: 150,
  ...
  Widget _body(BuildContext context) {
    if (isLoading) {
      return const _RouteShapeMessage(
        key: Key('ghost-route-shape-loading'),
        message: '경로를 불러오는 중',
      );
    }
    if (errorMessage != null) {
      return _RouteShapeMessage(
        key: const Key('ghost-route-shape-fallback'),
    ...
    if (routePoints == null || routePoints.length < 2) {
      return const _RouteShapeMessage(
        key: Key('ghost-route-shape-fallback'),
    ...
    return CustomPaint(
      key: const Key('ghost-route-shape-layer'),
      painter: _GhostRouteShapePainter(routeSegments),
...
class _GhostRouteShapePainter extends CustomPainter {
  const _GhostRouteShapePainter(this.segments);
  ...
  @override
  bool shouldRepaint(covariant _GhostRouteShapePainter oldDelegate) {
